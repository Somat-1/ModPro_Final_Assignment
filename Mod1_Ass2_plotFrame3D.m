...
if ~exist('Finternal','var')
    ...
    for iTruss = 1:size(trusses,1)
        plot3(jointCoordinates(trusses(iTruss,:),1),jointCoordinates(trusses(iTruss,:),2),jointCoordinates(trusses(iTruss,:),3),'-k','LineWidth',2)
    end
else
    ...
    for iTruss = 1:size(trusses,1)
        plot3(jointCoordinates(trusses(iTruss,:),1),jointCoordinates(trusses(iTruss,:),2),jointCoordinates(trusses(iTruss,:),3),'-',...
            'LineWidth',2 + 3*abs(Finternal(iTruss))/Fmax,'Color',c(Finternal(iTruss)/Fmax))
    end
end

% Plot joints
plot3(jointCoordinates(:,1),jointCoordinates(:,2),jointCoordinates(:,3),'ok','MarkerFaceColor',[0 0 0],'MarkerSize',6)
...
for iSupport = 1:size(supportReactions,1)
    ...
    if supportReactions(iSupport,2) == 1
        ...
        quiver3(jointCoordinates(supportReactions(iSupport,1),1),jointCoordinates(supportReactions(iSupport,1),2),jointCoordinates(supportReactions(iSupport,1),3),...
            qL,0,0,'r','MaxHeadSize',1,'LineWidth',2)
        
    elseif supportReactions(iSupport,2) == 2
        % If support reaction in y-direction
        % quiver is a function to plot arrows
        quiver3(jointCoordinates(supportReactions(iSupport,1),1),jointCoordinates(supportReactions(iSupport,1),2),jointCoordinates(supportReactions(iSupport,1),3),...
            0,qL,0,'r','MaxHeadSize',1,'LineWidth',2)
        
    else
        % If support reaction in z-direction
        % quiver is a function to plot arrows
        quiver3(jointCoordinates(supportReactions(iSupport,1),1),jointCoordinates(supportReactions(iSupport,1),2),jointCoordinates(supportReactions(iSupport,1),3),...
            0,0,qL,'r','MaxHeadSize',1,'LineWidth',2)
    end
end
...
for iExt = 1:size(externalForces,1)
    ...
    if externalForces(iExt,2) == 1
        ...
        % quiver is a function to plot arrows
        quiver3(jointCoordinates(externalForces(iExt,1),1),jointCoordinates(externalForces(iExt,1),2),jointCoordinates(externalForces(iExt,1),3),...
            qL,0,0,'b','MaxHeadSize',1,'LineWidth',2)
    elseif externalForces(iExt,2) == 2
        % If external force in y-direction
        % quiver is a function to plot arrows
        quiver3(jointCoordinates(externalForces(iExt,1),1),jointCoordinates(externalForces(iExt,1),2),jointCoordinates(externalForces(iExt,1),3),...
            0,qL,0,'b','MaxHeadSize',1,'LineWidth',2)
    else
        % If external force in z-direction
        % quiver is a function to plot arrows
        quiver3(jointCoordinates(externalForces(iExt,1),1),jointCoordinates(externalForces(iExt,1),2),jointCoordinates(externalForces(iExt,1),3),...
            0,0,qL,'b','MaxHeadSize',1,'LineWidth',2)
    end
end

% Set the scaling of the x-, y-axis and z-axis to equal
axis equal
% Zoom out
