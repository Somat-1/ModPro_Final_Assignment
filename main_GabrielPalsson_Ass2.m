...
truss_definition_GabrielPalsson_Ass2;
...
deltaZ=jointCoordinates(jJoint,3)-jointCoordinates(iJoint,3);
...
if (supportReactions(iSupport,2)==1);
    ...
    if (supportReactions(iSupport,2)==2);
    A(iJoint+nJoints,nTrusses+iSupport)=1;
else
    A(iJoint+2*nJoints,nTrusses+iSupport)=1  ;  
end
end
end
...
if (externalForces(iExtForce,2)==1);
    ...
    if (externalForces(iExtForce,2)==2);
    b(iJoint+nJoints,1)=-externalForces(iExtForce,3);
else
    b(iJoint+2*nJoints,1)=-externalForces(iExtForce,3);
    end
end
